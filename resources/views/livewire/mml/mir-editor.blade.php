<div>
    <x-page.container :breadcrumbs="[
        ...
        @if (! empty($nivelesSinEficacia))
            <div class="mb-4 rounded-md border-l-4 border-l-amber-400 bg-amber-50 p-4">
                <div class="flex">
                    ...
                    <div class="ml-3">
                        <h3 class="text-sm font-medium text-amber-800">
                            Niveles sin indicador de EFICACIA
                        </h3>
                        <p class="mt-1 text-sm text-amber-700">
                            La metodología MIR exige que cada nivel tenga al menos un indicador de dimensión EFICACIA.
                            Los siguientes niveles aún no cumplen:
                        </p>
                        <ul class="mt-2 list-inside list-disc text-sm text-amber-700">
                            @foreach ($nivelesSinEficacia as $nivel)
                                <li>
                                    <span class="font-medium">{{ $nivel['tipo_nivel'] }}</span>@if (! empty($nivel['resumen'])): {{ \Illuminate\Support\Str::limit($nivel['resumen'], 80) }}@endif
                                </li>
                            @endforeach
                        </ul>
                    </div>
