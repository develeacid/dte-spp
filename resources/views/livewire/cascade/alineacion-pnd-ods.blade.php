    ...
    @if($showForm)
        <div class="bg-gray-50 rounded-lg p-6 border-2 border-dashed border-gray-300">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                {{-- Selector PND --}}
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Objetivo PND</label>
                    <input type="text"
                ...
                {{-- Selector ODS --}}
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Meta ODS</label>
                    <input type="text"
                        ...
                        <div class="mt-2 bg-white border rounded-md shadow-sm max-h-48 overflow-y-auto">
                            @foreach($this->ods_resultados as $ods)
                                <button wire:click="selectOds({{ $ods->id }})"
                                    ...
                                    <div class="text-sm font-medium text-gray-900">{{ $ods->clave }}</div>
                                    <div class="text-xs text-gray-500">ODS {{ $ods->objetivo->numero }}: {{ Str::limit($ods->descripcion, 40) }}</div>
                                </button>
                            @endforeach
                        </div>
                    @endif
                ...
                @foreach($this->alineaciones as $pnd)
                    <div class="bg-gray-50 rounded-lg p-4">
                        <div class="flex items-center space-x-2 mb-2">
                            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-100 text-green-800">PND</span>
                            <span class="text-sm font-medium text-gray-900">{{ $pnd->clave }}</span>
                        </div>
                        <p class="text-sm text-gray-600 mb-3">{{ Str::limit($pnd->descripcion, 100) }}</p>
