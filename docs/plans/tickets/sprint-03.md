...
**Decisión técnica:** Los prompts no se hardcodearán. Se crearán como archivos Blade en `resources/views/prompts/` y se cargarán dinámicamente (ej. `view('prompts.mml.problem_validation', ['text' => $text])->render()`). Esto permite versionarlos y modificarlos sin tocar el código del servicio.
...
- [ ] Creación de la carpeta `resources/views/prompts/` con al menos un prompt de ejemplo.
