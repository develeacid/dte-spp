...
{
    /**
     * Agrega `ods_claves` (jsonb de claves ODS) a la vista para que el publisher
     * DS-05 pueda llenar `pub_alineacion_estrategica.ods_metas` (array de claves).
     */
    ...
    private function viewSql(bool $withOdsClaves): string
    {
        $odsClaves = $withOdsClaves
            ? ', jsonb_agg(DISTINCT odm.clave) FILTER (WHERE odm.clave IS NOT NULL) AS ods_claves'
            : '';
        ...
        return "
            ...
            SELECT
                ...
                string_agg(DISTINCT odm.descripcion, '; ')   AS ods_metas{$odsClaves}
